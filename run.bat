...
echo [INFO] Iniciando X-BLAST Enterprise v2.0 desde el directorio principal...
echo [INFO] Interprete: !PY_EXE!
echo.

cd /d "%~dp0VOLADURA_PRO_10X"
"!PY_EXE!" "main.py"

if errorlevel 1 (
    echo.
    echo [AVISO] La aplicacion se cerro con codigo de error %errorlevel%.
    echo Si faltan dependencias, ejecuta desde la raiz: pip install -r requirements.txt
    echo.
    pause
)
